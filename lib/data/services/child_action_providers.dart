...
class ChildActionService {
  ...
  /// Marca uma tarefa como feita pela criança.
  ///
  /// Correção importante:
  /// - Antes o log era criado com ID aleatório.
  /// - Agora o ID é fixo por criança + tarefa + data.
  /// - Isso impede duplicidade da mesma tarefa no mesmo dia.
  Future<void> completeTask(
    ...
    final logId = '${childId}_${taskId}_$todayStr';
    final logRef = _db.collection('task_logs').doc(logId);

    await _db.runTransaction((transaction) async {
      final existingLog = await transaction.get(logRef);
  ...
  Future<bool> buyReward(ChildModel child, RewardModel reward) async {
    ...
    batch.update(childRef, {
      'currentXp': FieldValue.increment(-reward.xpCost),
    });
...
final childActionServiceProvider = Provider<ChildActionService>((ref) {
  return ChildActionService();
});
...
/// Provider principal da tela da criança.
///
/// Este provider substitui o uso antigo de `todayTasksStreamProvider`
/// para evitar conflito de nomes com `firestore_providers.dart`.
final childTodayTasksProvider =
  ...
  final completedTaskIdsToday = allLogs
      .where((log) => log['taskId'] != null)
      .map((log) => log['taskId'].toString())
      .toSet();
  ...
  final pendingTasksToday = allTasks.where((task) {
    ...
    if (taskDate.isAfter(todayStart)) {
      return false;
    }
  ...
  pendingTasksToday.sort((a, b) => a.time.compareTo(b.time));
